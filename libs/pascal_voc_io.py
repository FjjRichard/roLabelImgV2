...
class PascalVocWriter:
    ...
    def addRotatedBndBox(self, cx, cy, w, h, angle, robndFourbox, name, difficult):
        x1, y1, x2, y2, x3, y3, x4, y4 = robndFourbox
        robndbox = {
                'cx': cx, 
                'cy': cy, 
                'w': w, 
    ...
    def appendObjects(self, top):
        ...
        for each_object in self.roboxlist:
            ...
            segmentation = SubElement(object_item, 'segmentation')

            x1 = SubElement(segmentation, 'x1')
            x1.text = str(each_object['x1'])

            y1 = SubElement(segmentation, 'y1')
            y1.text = str(each_object['y1'])

            x2 = SubElement(segmentation, 'x2')
            x2.text = str(each_object['x2'])

            y2 = SubElement(segmentation, 'y2')
            y2.text = str(each_object['y2'])

            x3 = SubElement(segmentation, 'x3')
            x3.text = str(each_object['x3'])

            y3 = SubElement(segmentation, 'y3')
            y3.text = str(each_object['y3'])

            x4 = SubElement(segmentation, 'x4')
            x4.text = str(each_object['x4'])

            y4 = SubElement(segmentation, 'y4')
