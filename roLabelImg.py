...
from convertCoco import ConvertCoCo
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None):
        ...
        self.sl.valueChanged.connect(self.slchangevalue)

        self.sltext = QLabel()
        self.sltext.setText(f"train:val = {self.sl.value()/100.0}:{round((1-self.sl.value()/100.0),2)}")

        self.convertBt = QPushButton("Convert")
        self.convertBt.clicked.connect(self.convertbt)
        self.convertBt.setEnabled(False)
        convertLayout = QVBoxLayout()
        convertLayout.setContentsMargins(0, 0, 0, 0)
        convertLayout.addWidget(self.sltext)
        convertLayout.addWidget(self.sl)
        convertLayout.addWidget(self.convertBt)
        
        convertContainer = QWidget()
        convertContainer.setLayout(convertLayout)
        self.convertdock = QDockWidget(u'Convert CoCo', self)
        self.convertdock.setObjectName(u'Convert')
        self.convertdock.setWidget(convertContainer)
        ...
        self.addDockWidget(Qt.RightDockWidgetArea, self.convertdock)
    ...
    def convertbt(self):
        ...
        #         convert = ConvertCoCo(imgFileDir,imgFileDir)
        #     # imgFileName = os.path.basename(self.filePath)
        #     # savedFileName = os.path.splitext(imgFileName)[0] + XML_EXT
    ...
    def changeSavedir(self, _value=False):
        ...
        self.statusBar().show()
        self.convertBt.setEnabled(True)
